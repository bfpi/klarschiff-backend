...
public class DbSyncService {
	...
	/**
	 * Liest das SQL-Script mit den Trigger und Triggerfunktionen zum Synchronisieren der Frontend- und BackendDB, wobei die
	 * Platzhalter <code>%host%</code>, <code>%port%</code>, <code>%dbname%</code>, <code>%username%</code> und 
	 * <code>%password%</code> entsprechend durch Werte ersetzt werden.  
	 * @return SQL-Script mit den Trigger und Triggerfunktionen zum Synchronisieren der Frontend- und BackendDB
	 */
	...
	public void executeSqlScriptFrontendDb(SqlScriptUtil.State state) throws HibernateException {
		...
		try {
			...
			try { session.close(); } catch (Exception ex) {}
			try { sessionFactory.close(); } catch (Exception ex) {}
		}
	}
