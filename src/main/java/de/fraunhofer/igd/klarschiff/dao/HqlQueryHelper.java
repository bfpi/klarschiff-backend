...
/**
 * Mit Hilfe der Klasse wird die Erstellung von korrekten HQL-Anfragen vereinfacht. Dabei werden die
 * Angaben wie Projektion (SelectAttributes), Selektion (WhereConditions), Gruppierung
 * (GroupByAtttributes) etc. separat nacheinander angegeben. Die HQL-Anfrage kann dann mit der
 * Methode <code>getHqlQuery()</code> ermittelt werden. Analog dazu kann die Anfrage an die DB
 * direkt mit Hilfe der Funktion <code>getResultList(EntityManger)</code> oder
 * <code>getSingleResult(EntityManger)</code> gestellt werden.
 *
...
public class HqlQueryHelper {
  ...
  public String getHqlQuery(EntityManager entityManager) {
    ...
    if (fromTables.size() < 1) {
      throw new RuntimeException("Es sind keine Tabellen angegeben");
    }
    ...
    if (!whereConditions.isEmpty()) {
      ...
      for (Iterator<String> iter = whereConditions.iterator(); iter.hasNext();) {
        str.append(iter.next());
        if (iter.hasNext()) {
          str.append(" " + whereConditionsOperation + " ");
        }
      }
    }
