...
public class PropertyPlaceholderConfigurer extends org.springframework.beans.factory.config.PropertyPlaceholderConfigurer {
  ...
  /**
   * Ermittelt das verwendet Profile über die Umgebungsvariable <code>KLARSCHIFF_HRO_PROFILE</code>
   * des Rechners, die Java-Umgebungsvariable <code>KLARSCHIFF_HRO_PROFILE</code> oder dem Property
   * <code>profile</code> in der <code>settings.properties</code>.
   *
   * @return Profile
   */
  protected static String getProfile() {
    ...
    try {
      ...
      if (StringUtils.isBlank(_profile)) {
        logger.info("Profil wurde mit System.getenv('" + PROFILE_SYSTEM_PROPERTY_NAME + "') gefunden.");
      }

      if (StringUtils.isBlank(_profile)) {
        _profile = System.getProperty(PROFILE_SYSTEM_PROPERTY_NAME);
        if (!StringUtils.isBlank(_profile)) {
          logger.info("Profil wurde mit System.getProperty('" + PROFILE_SYSTEM_PROPERTY_NAME + "') gefunden.");
        }
      }
      ...
      if (StringUtils.isBlank(_profile)) {
        ...
        if (!StringUtils.isBlank(_profile)) {
          logger.info("Profil wurde in " + SETTINGS_FILE_LOCATION + " gefunden.");
        }
      }
