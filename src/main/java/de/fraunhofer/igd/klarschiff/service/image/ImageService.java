 ...
 * Bildbereichen, bzw. zum Auslesen eines Bildes aus einem HTTP-Request
 *
...
public class ImageService {
  ...
  /**
   * Gibt an, wie ein Bild skaliert werden soll. Das Ergebnisbild hat nach der Skalierung die
   * entsprechenden maximalen Abmaße bei <code>max</code>. Das Ergebnisbild hat nach der Skalierung
   * genau die Abmaße haben soll, wobei entsprechende Teile des Bildes transparent gemacht wurden
   * bei <code>correct</code>
   */
  public enum ScaleTyp {
  ...
  /**
   * Speichert das in <code>image</code> übergebene Bild in den Dateinamen, der über
   * <code>prevFilename</code> übergeben wurde. Falls dieser leer ist, wird mittels
   * <code>vorgang</code>-ID und <code>middlePart</code> und einer UUID ein neuer Dateiname
  ...
  /**
   * Schwarz Bereiche in einem Bild aus. Das Vorschau- als auch das eigentliche Bild eines Vorganges
   * wird dabei geändert.
   *
   * @param vorgang Vorgang, bei dem Bereiche aus dem Bild geschwarzt werden sollen
   * @param rectangles Liste von Rechtecken als String, die für die Schwarzung verwendet werden
   * sollen
  ...
  /**
   * Scalliert ein Bild. Dabei kann über den ScaleType angegeben werden, ob das Ergebnisbild die
   * entsprechenden maximalen Abmaße haben soll, oder das Ergebnisbild genau die Abmaße haben soll,
   * wobei entsprechende Teile des Bildes dann transparent gemacht werden sollen.
   *
