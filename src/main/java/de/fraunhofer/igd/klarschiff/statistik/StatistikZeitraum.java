...
public class StatistikZeitraum extends StatistikCommon {
  ...
  private void updateSheetValuesKategorien(Sheet sheet, StatistikCommand cmd, HashMap daten) throws ParseException {
    ...
    while (next_department) {
      ...
      if (kategorien.size() > 0) {
        ...
        cell.setCellFormula("SUM(J" + (current_row + 2) + ":J" + (current_row + 2 + kategorien.size()) + ")");
      } else {
  ...
  private HashMap getData(StatistikCommand cmd) throws ParseException {
    ...
    List<Object[]> vormonateKategorien = statistikDao.getAnzahlOffeneVorgaengeBis(cmd.getTyp(), cZeitraumVon.getTime());
    zusammenfassung = mergeResults(zusammenfassung, "vormonate", vormonateKategorien);

    List<Object[]> neueKategorien = statistikDao.getAnzahlNeueVorgaengeInZeitraum(cmd.getTyp(), cmd.getZeitraumVon(), cmd.getZeitraumBis());
    zusammenfassung = mergeResults(zusammenfassung, "neu", neueKategorien);

    List<Object[]> abgeschlosseneKategorien = statistikDao.getAnzahlAbgeschlosseneVorgaengeInZeitraum(cmd.getTyp(), cmd.getZeitraumVon(), cmd.getZeitraumBis());
